...
export default function PoolDetailsPage() {
  ...
  useEffect(() => {
    ...
    load().catch(e => setError(e instanceof Error ? e.message : 'Não foi possível carregar o Pool.'));
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      await load();
      setEditing(false);
      setSuccess('Pool atualizado com sucesso.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Falha ao atualizar o Pool.');
    } finally { setSaving(false); }
  }

  async function remove() {
    if (!pool || !window.confirm(`Excluir o Pool "${pool.name}"? Pools com ativos, pastas ou perfis de estoque não podem ser excluídos.`)) return;
    setError(''); setSuccess('');
    try {
      await api(`/pools/${pool.id}`, { method: 'DELETE' });
      router.replace('/pools');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Falha ao excluir o Pool.');
    }
  }

  return <>
    <Link className="table-action asset-detail-back" href="/pools">Voltar aos Pools</Link>
    ...
    {pool && <>
      <div className="page-head asset-detail-page-head">
        <div className="page-head-content"><div className="eyebrow">Pool</div><h1>{pool.name}</h1><div className="page-description">{pool.description || 'Sem descrição cadastrada.'}</div></div>
      ...
      <section className="card section-card"><div className="section-heading"><div><h2>Ações rápidas</h2><p>Ativos e componentes permanecem separados para facilitar a consulta.</p></div></div><div className="pool-quick-actions"><Link className="primary button-link" href={`/assets?poolId=${encodeURIComponent(pool.id)}`}>Ver ativos deste Pool</Link><Link className="secondary-link" href={`/stock?poolId=${encodeURIComponent(pool.id)}`}>Ver estoque deste Pool</Link></div></section>

      {editing && <div className="modal-backdrop" onMouseDown={() => setEditing(false)}><div className="modal-card" onMouseDown={e => e.stopPropagation()}><div className="modal-heading"><div><div className="eyebrow">Pool</div><h2>Editar {pool.name}</h2><p>Altere identificação, descrição ou status do agrupamento.</p></div><button type="button" className="modal-close" onClick={() => setEditing(false)}>×</button></div><form className="stack-form" onSubmit={save}><div className="form-field"><label>Nome *</label><input required minLength={2} maxLength={100} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div><div className="form-field"><label>Descrição</label><textarea rows={4} maxLength={4000} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></div><label className="toggle-row"><input type="checkbox" checked={form.active} onChange={e => setForm({ ...form, active: e.target.checked })} /><span><strong>Pool ativo</strong><small>Ao inativar, Gestores e Visualizadores deixam de enxergar o Pool até a reativação.</small></span></label><div className="form-actions-row"><button type="button" className="secondary" onClick={() => setEditing(false)}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Salvando...' : 'Salvar alterações'}</button></div></form></div></div>}
    </>}
    {!pool && !error && <div className="empty-state">Carregando Pool...</div>}
  </>;
}
