...
export default function StockPage() {
  ...
  return <>
    <div className="page-head"><div><div className="eyebrow">Componentes por setor</div><h1>Estoque e componentes</h1><div className="page-description">Gerencie peças, componentes, saldos disponíveis e unidades instaladas. Patrimônios principais ficam em Ativos.</div></div>
      <Field label="Setor"><select value={poolId} onChange={e => changePool(e.target.value)}><option value="">Todos os meus Setores</option>{pools.map(p => <option key={p.id} value={p.id}>{p.name}{p.active ? '' : ' (inativo)'}</option>)}</select></Field></div>
    {(error || success) && <div role="status" className={`notice ${error ? 'notice-error' : 'notice-success'}`}>{error || success}</div>}
    {!pools.length && !loading && <div className="notice notice-info">Nenhum Setor disponível. Solicite ao administrador a liberação dos seus Setores em Usuários.</div>}
    ...
    {tab === 'catalog' && <>
      ...
      <section className="card section-card"><div className="section-heading"><div><h2>Perfis de itens</h2><p>Crie, por exemplo, um perfil de memória com capacidade e especificação técnica.</p></div>{canEdit && <button type="button" disabled={!poolId} onClick={() => { setEditing(''); setProfile({ ...blankProfile, categoryId }); setShowProfile(!showProfile); }}>+ Novo perfil</button>}</div>
        {canEdit && !poolId && <p className="field-help">Selecione um Setor no topo para criar perfis ou movimentar unidades.</p>}
        ...
        <div className="table-wrap"><table><thead><tr><th>Perfil / especificações</th><th>Setor</th><th>Categoria</th><th>Disponível</th><th>Instalado</th><th>Total</th>{canEdit && <th>Ações</th>}</tr></thead><tbody>
    ...
    {tab === 'assign' && canEdit && <section className="card section-card"><div className="section-heading"><div><h2>Associar componentes a ativos existentes</h2><p>Selecione os equipamentos e a quantidade por equipamento. O lote inteiro é validado antes da gravação.</p></div></div>
      {!poolId ? <div className="notice notice-info">Selecione um Setor no topo.</div> : <form onSubmit={assign}>
