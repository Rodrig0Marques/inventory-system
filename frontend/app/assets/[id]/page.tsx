...
export default function AssetDetails() {
  ...
  const [error, setError] = useState(''), [success, setSuccess] = useState(''), [busy, setBusy] = useState(false), [editable, setEditable] = useState(false), [canStockManage, setCanStockManage] = useState(false);
  ...
  useEffect(() => { const session = getSessionUser(); setEditable(hasPermission('ASSET_EDIT', session)); setCanStockManage(hasPermission('STOCK_MANAGE', session)); load().catch(e => setError(e.message)); }, [load]);
