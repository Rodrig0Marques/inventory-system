...
export default function AssetDetails() {
  ...
  return <>
    <Link className="table-action" href="/assets">Voltar aos ativos</Link>
    ...
    {asset && <><div className="page-head"><div><div className="eyebrow">{asset.patrimonyNumber}</div><h1>{asset.name}</h1><div className="page-description">{asset.pool.name} / {asset.category.name}{asset.folder ? ` / ${asset.folder.name}` : ''}</div></div>{editable && <Link className="primary button-link" href={`/stock?poolId=${asset.poolId}&assetId=${id}`}>Associar componentes</Link>}</div>
