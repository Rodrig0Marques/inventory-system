...
const select = { id: true, name: true, email: true, role: true, active: true, canGlobalAssetLookup: true, canGlobalDashboardStats: true, createdAt: true, updatedAt: true,
...
const fields = z.object({ name: z.string().trim().min(2).max(200), email: z.string().trim().email().max(254), role: z.nativeEnum(UserRole), active: z.boolean(), canGlobalAssetLookup: z.boolean(), canGlobalDashboardStats: z.boolean(), poolIds: z.array(z.string().min(1)).max(500) });
...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.post('/', async (request, reply) => {
    const data = fields.extend({ password, role: fields.shape.role.default(UserRole.VIEWER), active: z.boolean().default(true), canGlobalAssetLookup: z.boolean().default(false), canGlobalDashboardStats: z.boolean().default(false), poolIds: fields.shape.poolIds.default([]) }).parse(request.body);
    ...
    const user = await serial(async tx => {
      ...
      const result = await tx.user.create({ data: { name: data.name, email: data.email.toLowerCase(), password: hash, role: data.role, active: data.active,
        ...
        canGlobalDashboardStats: data.canGlobalDashboardStats,
